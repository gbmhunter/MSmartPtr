...
# @brief 			Makefile for Linux-based make, to compile the MSmartPtr library, example code and run unit test code.
# @details
...
src : deps $(SRC_OBJ_FILES)
	# Make library
	ar r libMSmartPtr.a $(SRC_OBJ_FILES)
...
test : deps $(TEST_OBJ_FILES) | src
	...
	g++ $(TEST_LD_FLAGS) -o ./test/Tests.elf $(TEST_OBJ_FILES) -L./ -lMSmartPtr $(DEP_LIB_PATHS) $(DEP_LIBS)
